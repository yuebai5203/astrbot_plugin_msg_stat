...
class MsgStat(Star):
    ...
    async def _resolve_names(self, platform_id: str, group_ids: list[str], user_ids: list[str]):
        ...
        for kind, id_ in pending:
            if kind == "group":
                coros.append(bot.call_action("get_group_info", group_id=int(id_)))
            else:
                coros.append(bot.call_action("get_stranger_info", user_id=int(id_)))

        results = await asyncio.gather(*coros, return_exceptions=True)
        for (kind, id_), result in zip(pending, results):
